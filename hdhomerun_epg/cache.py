    ...
    def _init_db(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """)
                conn.execute("CREATE INDEX IF NOT EXISTS idx_end_time ON epg_chunks (end_time)")
        except Exception as e:
            logger.error(f"🚨 Failed to initialize cache DB: {e}")

    def get_chunk(self, start_time: int, ttl_seconds: int = 86400) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    ...
                    if age < ttl_seconds:
                        logger.debug(f"✅ Cache HIT for chunk {start_time} (Age: {age}s)")
                        decompressed = gzip.decompress(data_blob)
                        return json.loads(decompressed)
                    else:
                        logger.debug(f"🍂 Cache STALE for chunk {start_time} (Age: {age}s)")
                        return None
